...
const Table = <T extends Record<string, any>>({
  ...
  const renderCell = (row: T, column: Column<T>, rowIndex: number) => {
    ...
    if (column.type === "status" && column.statusConfig) {
      ...
      const config = column.statusConfig.values[status] ||
        column.statusConfig.values["default"] || {
    ...
    if (column.type === "payment" && column.paymentConfig) {
      const status = value?.toString().toLowerCase() || "";
      const config = column.paymentConfig.values[status] ||
        column.paymentConfig.values["default"] || {
          label: value?.toString() || "-",
          color: isDarkMode ? "#9CA3AF" : "#6B7280",
          bgColor: isDarkMode ? "#374151" : "#F3F4F6",
        };
                    ...
                    >
                      {rowsPerPageOptions.map((option) => (
                        <div
                          ...
                          className={rowsPerPageOptionClasses(
                            option === rowsPerPage,
                          )}
                        >
                          {option} per page
          ...
          >
            ...
            >
              ...
              <div className={modalBodyClasses}>
                {editingCell.column.type === "status" &&
                editingCell.column.statusConfig ? (
                  <div className="space-y-3">
                    {Object.entries(editingCell.column.statusConfig.values).map(
                      ([key, config]) => (
                        <button
                          key={key}
                ...
                ) : editingCell.column.type === "payment" &&
                  editingCell.column.paymentConfig ? (
                  <div className="space-y-3">
                    {Object.entries(
                      editingCell.column.paymentConfig.values,
                    ).map(([key, config]) => (
                      <button
                        key={key}
                        onClick={() => setSelectedValue(key)}
